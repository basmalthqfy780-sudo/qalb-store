...
تصدير `/admin/orgs.csv` بثمانية أعمدة (`code,org,email,tier,status,issued,expires,seats,used`) فيه بريدُ الجهة وحده؛ بريد الطالب يبقى داخل الطلب ولا يخرج إلى اللوحة. `orgs.jsonl` مقروءٌ بوضعية `0600` كغيره، ويُختَم عند الإقلاع. القوائم والتعديل عبر `GET /admin/orgs` و`PATCH /admin/orgs/:code` (`seats` لا ينزل تحت ما استُهلك، و`months` يجدّد الاستحقاق) و`POST /admin/orgs` يُصدر رمزًا بحدٍّ أقصى عشرةً في الطلب. بوابةُ المتجر تُطفأ بمفتاح `QALB_B2B=off` وحده.
